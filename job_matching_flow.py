...
class YoinStructured(BaseModel):
    ...
    受信日時: str = Field(description="受信日時")
    氏名: str = Field(description="")
    年齢: str = Field(description="")
    スキル: str = Field(description="")
    最寄駅: str = Field(description="")
    勤務形態: str = Field(description="")
    単価: str = Field(description="")
    備考: str = Field(description="")
    件名: str = Field(description="")

class AnkenStructured(BaseModel):
    id: str = Field(description="ID")
    受信日時: str = Field(description="受信日時")
    案件名: str = Field(description="")
    必須スキル: str = Field(description="")
    作業場所: str = Field(description="")
    勤務形態: str = Field(description="")
    期間: str = Field(description="")
    単価: str = Field(description="")
    備考: str = Field(description="")
    件名: str = Field(description="")
...
- idと受信日時は絶対に入力値と一致させてください。
""")
    
    chain = prompt | llm | JsonOutputParser(pydantic_object=YoinStructured)
...
注意:
- "必須スキル"は箇条書きに変換し、各行の頭に"- "を付加してください
- "期間"は「期間」「時期」「開始」「作業開始時期」「作業期間」「スケジュール」などで示されている
- 出力は必ず上記のJSONのみで返してください。
- コードブロックや説明文は不要です。
- idと受信日時は絶対に入力値と一致させてください。
""")
    
    chain = prompt | llm | JsonOutputParser(pydantic_object=AnkenStructured)
    result = chain.invoke({"input_text": raw_text})
    return result
...
def format_yoin_flow(params: Dict[str, Any]):
    ...
    for record in records:
        ...
        print(f"Processed yoin ID: {structured['id']}")
...
def format_anken_flow(params: Dict[str, Any]):
    ...
    for record in records:
        ...
        print(f"Processed anken ID: {structured['id']}")
...
- 推奨アクションは、担当者として検討すべき項目や行動を示してください
""")
